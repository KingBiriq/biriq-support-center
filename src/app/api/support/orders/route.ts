...
export async function GET(req: NextRequest) {
  try {
    ...
    await requirePermission(session.staffId, 'orders.view');
    ...
    // Payload: timestamp.requestId.?searchParams
    ...
    const targetUrl = new URL("/api/internal/support/orders", baseUrl);
    ...
    if (!response.ok) {
      ...
      let errMsg = "Failed to fetch orders from store";
      try {
